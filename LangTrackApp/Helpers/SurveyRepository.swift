...
struct SurveyRepository {
    ...
    //static let ltaUrl = "http://ht-lang-track.easterbunny-school.edu/api/"
    //url on Firebase: "https://lta.easterbunny-school.edu/api/"
    //stagingUrl on Firebase: "https://lta-staging.easterbunny-school.edu/api/"
        ...
        if selectedAssignment != nil{
            if selectedAssignment!.id != ""{
                getUrl { (theUrl) in
                    if let theUrl = theUrl{
                        if theUrl != ""{
                            //https://lta.easterbunny-school.edu/api/assignments/(selectedAssignment!.id)/open
        ...
        getUrl { (theUrl) in
            if let theUrl = theUrl{
                if theUrl != ""{
                    ...
                    if userId != ""{
                        //http://ht-lang-track.easterbunny-school.edu/api/users/alicia/assignments
    ...
    private static func createAssignmentsFromData(data: Data) -> [Assignment]?{
        ...
        do {
            ...
            for (_,assignmentJson):(String, JSON) in json {
                ...
                //if expiry is empty -> set expiry to one hour from publish
                if tempAssignment.expiry == ""{
                    if let publishDate = DateParser.getDate(dateString: tempAssignment.published){
                        var dateComponent = DateComponents()
                        dateComponent.hour = 1
                        if let futureDate = Calendar.current.date(byAdding: dateComponent, to: publishDate){
                            tempAssignment.expiry = DateParser.getDateString(for: futureDate)
                        }
                    }
                }
